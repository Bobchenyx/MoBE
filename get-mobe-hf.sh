...
python -u get_hf_model.py --base_model $MODEL_DIR/$MODEL_NAME \
      --mobe_dir $MODEL_DIR/MoBE/$MODEL_NAME \
      --save_dir $MODEL_DIR/MoBE/$MODEL_NAME-MoBE-hf \
      --start_layer 0 \
      --end_layer 48 \
      --num_experts 128 \
      --dtype bfloat16 2>&1 | tee logs/get_mobe_hf_$MODEL_NAME.log
